...
separated_path = path.join(
    getcwd(), "separated", "htdemucs", path.basename(PATH).rsplit(sep=".", maxsplit=1)[0], "vocals.wav"
)
...
vocal_numpy: np.ndarray = np.array(vocal.get_array_of_samples()).astype(np.int16).reshape((-1, vocal.channels))
...
f, t, Sxx = signal.stft(vocal_merge, vocal.frame_rate, window=win, nperseg=win.shape[0], scaling="spectrum",
...
vocal_ave = np.convolve(vocal_sum, np.ones(ave_size) / ave_size, mode="same")
vocal_gaussian_norm = (vocal_ave - vocal_ave.min()) / (vocal_ave.max() - vocal_ave.min())
...
base_sound = vocal
base_numpy: np.ndarray = np.array(base_sound.get_array_of_samples()).astype(np.int16).reshape((-1, base_sound.channels))

vocal_mute = np.tile(no_music.reshape((-1, 1)),
                     (1, int((base_sound.frame_rate / vocal.frame_rate) * (win.shape[0] / 2)))).reshape((-1,)) \
                 .astype(np.float16)[: base_numpy.shape[0]]
mute_size = int(ave_size * base_sound.frame_rate / 20)
vocal_mute_smoothed = cupy.convolve(cupy.array(vocal_mute), cupy.array(np.ones(mute_size) / mute_size), mode="same")
...
print(muted.dtype)
print(muted.shape)
muted_audio = AudioSegment(muted.astype(numpy.int16).tobytes(), sample_width=base_sound.sample_width,
                           frame_rate=base_sound.frame_rate, channels=base_sound.channels)
muted_audio.export("muted.wav", "wav")
# pyplot.plot(muted[:, 0])
# pyplot.show()
